...
function placeTrackedContent(box, snap) {
...
function inferFrame(now) {
  if (!targetReady || inFlight || !tracker || !['tracking', 'initializing'].includes(state)) return;
  ...
  trackerWork.then(result => {
    ...
    lastResultAt = performance.now();
    if (debugEnabled) $('debug').textContent = 'engine: NanoTrack / ' + currentTracker.mode
    ...
    if (!result.reliable || !isValidBox(result.box, frame.width, frame.height)) {
      ...
      targetBox = null;
      hideTarget();
      status('Checking selection');
      if (misses >= CONFIG.maxMisses) loseTracking();
      return;
    }
    misses = 0;
    ++hits;
    ...
    if (hits >= CONFIG.revealFrames) {
      ...
      sessionCopy('Your object, augmented', 'Move slowly. The toaster follows position and size.');
      placeTrackedContent(targetBox, first);
    }
  }).catch(error => {
...
async function boot() {
  try {
    ...
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) return;
      clearDrawing();
      if (['tracking', 'initializing'].includes(state)) loseTracking('You returned to the camera. Select your object again.');
    });
